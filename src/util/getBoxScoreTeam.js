...
export default function GetBoxScoreTeam(datas, connection_id) {
  let stats={
    "2P_A":	0,
    "2P_M":	0,
    "3P_A":	0,
  ...
  for (let i = 0; i < datas.length; i ++) {
    ...
    if (data["sub_tag"] == "2P") {
      ...
      if (data["sub_tag"] == "In") {
        sec_in = sec_in + Number(data["game_time"].substr(0,2))*60 + Number(data["game_time"].substr(4,2))
      } else if (data["sub_tag"] == "Out") {
        sec_out = sec_out + Number(data["game_time"].substr(0,2))*60 + Number(data["game_time"].substr(4,2))
      }
    } else {
  ...
  stats["play_time"] = String(stats["min"]).padStart(2,"0") + ":" + String(stats["sec"]).padStart(2,"0")

  const return_data = [[stats["PTS"],stats["3P_M"],stats["3P_A"],stats["2P_M"],stats["2P_A"],stats["Paint_M"],stats["Paint_A"],stats["Mid_M"],stats["Mid_A"],stats["FT_M"],stats["FT_A"],stats["Foul"],stats["OFF_REB"],stats["DEF_REB"],stats["TOT_REB"],stats["Assist"],stats["Steal"],stats["BLOCK"],stats["TO"],stats["play_time"]],[pts_off_steal,pts_off_or]]
